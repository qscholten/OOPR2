    ...
    ui(new Ui::MainWindow)
{
    ...
    kaarten[0]=std::make_shared<IdKaart>("20208294", "Qing", "Delft");
    kaarten[1]=std::make_shared<IdKaart>("4537580", "Qing", "Delft");
    KaartSlot* ks = dynamic_cast<KaartSlot*> (sloten[6].get());
    ks->voegIdKaartToe(kaarten[0].get());
    ks->voegIdKaartToe(kaarten[1].get());
...
void MainWindow::on_schuifdeurKnop_clicked() {
    if(deuren[0]->isDeurOpen()) {
        ...
        try {
            ...
            QMessageBox error;
            std::string message = "";
            if (exception.kaartVanBinnendringer().length() == 0) {
                message = "Geen idkaart voor ";
                message += exception.plaatsVanHetSlot();
            }
            else {
                message = "Exception bij het openen van een deur!";
                message += "\nIdKaart: ";
                message += exception.kaartVanBinnendringer();
                message += "\nSlotlocatie: ";
                message += exception.plaatsVanHetSlot();
            }
